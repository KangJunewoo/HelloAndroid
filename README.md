...
#### 요약
- 소켓을 사용해 인터넷으로 서버로 연결 만들고 데이터 주고받을 수 있다.
- 소켓 위에 헤더를 붙인게 http
- 웹서버는 시간관계상 만들어진걸로 대신함.
- 안드로이드에선 스레드와 핸들러가 기본이지만 volley를 사용하면 직접 스레드를 다루지 않아도 됨.
- json 응답을 받으면 gson으로 변환해 사용 가능.
- setText로 텍스트뷰에 보여주거나
- setImageURI로 이미지뷰에 보여주거나 ㄱㄴ.
- 일단 영화API에서 어떤 데이터를 넘겨주는지부터 분석해보자.
...
## 7. 멀티미디어
- 사진 동영상 음악 등등을 재생하고 보여주고 받아오고 등등 할 수 있다.
- 리싸이클러 : 리스트뷰와 달리 좌우로도 슥슥 넘길 수 있음.
...
- 액티비티 넘어갈 때 리스트뷰
- 햄버거 누르면 쭈루룩
- volley 통해 api에서 응답받은 것을 직접 반영하기 (계속 에러났던 부분)

#### 자바의 익숙치 않은 부분들
- 전역선언 : public static 붙이면 만사 오케이인가..?
- 파이썬 리스트같은걸로 ArrayList가 있는 건 알겠는데, 어떻게 하는거지? -> 클래스를 활용하면 되지 않을까?
- final 선언은 또 뭐지